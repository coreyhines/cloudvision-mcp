...
def convert_response_to_device_lifecycle(device) -> DeviceLifecycleSummary:
    try:
        _sw = DeviceSoftwareEoL(
            ...
            end_of_support=datetime_to_readable_format(
                device.value.software_eol.end_of_support.ToDatetime(), "with_day"
            ),
        )
    except Exception:
        _sw = DeviceSoftwareEoL(version="", end_of_support="")
    logging.debug(f"SW: {_sw}")
    try:
        _hw = DeviceHardwareEoL(
            end_of_life=datetime_to_readable_format(
                device.value.hardware_lifecycle_summary.end_of_life.ToDatetime(),
                "with_day",
            ),
            end_of_sale=datetime_to_readable_format(
                device.value.hardware_lifecycle_summary.end_of_sale.ToDatetime(),
                "with_day",
            ),
            end_of_tac_support=datetime_to_readable_format(
                device.value.hardware_lifecycle_summary.end_of_tag_support.ToDatetime(),
                "with_day",
            ),
            end_of_hardware_rma_request=datetime_to_readable_format(
                device.value.hardware_lifecycle_summary.end_of_hardware_rma_request.ToDatetime(),
                "with_day",
            ),
        )
    except Exception:
        ...
        for field, value in pb_obj.ListFields():
            ...
            try:
                ...
                if hasattr(value, "ListFields"):  # Nested protobuf message
                    result[field_name] = serialize_arista_protobuf(
                    ...
                    for item in value:
                        if hasattr(item, "ListFields"):  # Repeated protobuf messages
                            ...
                            result[field_name].append(
                                convert_protobuf_value(item, field)
                            )
...
def convert_protobuf_value(value, field_descriptor=None):
    ...
    if (
        field_descriptor
        and hasattr(field_descriptor, "enum_type")
        and field_descriptor.enum_type
    ):
        try:
